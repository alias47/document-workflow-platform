...
import type { CreateStaffData, StaffRole } from '../types/staff.types';
...
interface CreateStaffDialogProps {
  ...
  isPending: boolean;
  roles: StaffRole[];
  onConfirm: (data: CreateStaffData) => void;
  onCancel: () => void;
}

const EMPTY: CreateStaffData = {
  firstName: '',
  lastName: '',
  email: '',
  phone: '',
...
export function CreateStaffDialog({
  ...
  const [form, setForm] = useState<CreateStaffData>(EMPTY);
  const [confirmPassword, setConfirmPassword] = useState('');
  const [errors, setErrors] = useState<
    Partial<Record<keyof CreateStaffData | 'confirmPassword', string | undefined>>
  >({});

  if (!isOpen) return null;

  function field(key: keyof CreateStaffData) {
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          >
            <Field label="Password (leave blank for temp)" error={errors.password}>
              <input
                type="password"
                value={form.password ?? ''}
